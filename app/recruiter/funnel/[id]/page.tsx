...
export default async function FunnelPage({ params: paramsPromise, searchParams: searchParamsPromise }: { params: Promise<{ id: string }>; searchParams: Promise<{ thresholdApplied?: string; phase?: string; preselect?: string }> }) {
  const params = await paramsPromise;
  const searchParams = await searchParamsPromise;
  const user = await requireRole("recruiter");
  const funnel = await prisma.funnel.findUnique({ where: { id: params.id }, include: { drive: true } });
  if (!funnel || funnel.drive.ownerId !== user.id) return <Card>Funnel not found.</Card>;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mt-4">
        <LinkButton href={`/recruiter/drives/${funnel.driveId}`} className="btn-ghost">← Back to drive</LinkButton>
      </div>
    </div>
  );
}
