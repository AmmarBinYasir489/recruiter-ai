...
export default async function DriveDetail({ params: paramsPromise, searchParams: searchParamsPromise }: { params: Promise<{ id: string }>; searchParams: Promise<{ thresholdApplied?: string }> }) {
  ...
  return (
    <div>
          ...
          return (
            <Card key={f.id} hover>
              <div className="flex items-center justify-between">
                ...
                <Link href={`/recruiter/funnel/${f.id}`} className="btn-ghost text-xs">Open →</Link>
              </div>
              <p className="text-xs text-slate-500 mt-1">{stages.length} active phases · CV threshold {cv?.passScore ?? "—"}</p>
              <div className="flex flex-wrap gap-1 mt-2">
                {stages.map((s) => <span key={s.id} className="badge-muted text-xs">{s.type}</span>)}
              </div>
            </Card>
          );
        })}
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <LinkButton href={`/recruiter/candidates?driveId=${drive.id}`} className="btn-primary">Open all drive applicants ({drive._count.applications}) →</LinkButton>
      ...
      <Card className="text-sm text-slate-600">
        {drive._count.applications} applications. The full drive list includes unassigned, active, completed, and held candidates so staff can move or reassign any screened applicant to another published funnel.
      </Card>
    </div>
  );
}
