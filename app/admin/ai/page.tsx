...
import { aiProviderStates, environmentModelFor } from "@/lib/ai/config";
...
export default async function AdminAi() {
  ...
  const providerStates = aiProviderStates(settings);
  return (
    <div className="max-w-2xl">
      ...
      <AiSettingsForm provider={provider} model={settings?.model || environmentModelFor(provider) || defaultModelFor(provider)} providerStates={providerStates} />
    </div>
  );
}
