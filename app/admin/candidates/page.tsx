    ...
    <div>
      ...
      <div className="grid xl:grid-cols-[260px_minmax(0,1fr)] gap-6">
        <Card className="h-fit">
          ...
          <form className="space-y-3" method="get" action="/admin/candidates">
            <div>
            ...
            <Link href="/admin/candidates" className="btn-outline w-full">Clear filters</Link>
          </form>
          </CandidateFilters>
        </Card>
        ...
        <div className="min-w-0 w-full overflow-hidden">
          ...
          {views.length === 0 && <Link href="/admin/candidates" className="btn-outline mt-3">Clear filters</Link>}
          <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
            <span>Page {page.page} · {page.total} total · filtering runs on the full database.</span>
            <div className="flex gap-2">
              {page.page > 1 && <Link className="btn-ghost" href={`/admin/candidates?${new URLSearchParams({ ...Object.fromEntries(qs), page: String(page.page - 1) }).toString()}`}>← Previous</Link>}
              {page.page * page.pageSize < page.total && <Link className="btn-ghost" href={`/admin/candidates?${new URLSearchParams({ ...Object.fromEntries(qs), page: String(page.page + 1) }).toString()}`}>Next →</Link>}
            </div>
          </div>
        </div>
      </div>
