...
export default async function AdminDriveScores({ params: paramsPromise, searchParams: searchParamsPromise }: { params: Promise<{ id: string }>; searchParams: Promise<SP> }) {
            ...
            {rows.map((r, i) => (
              <tr key={r.applicationId} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                ...
                <td className="py-2 pr-3">
                  <Link href={`/admin/candidates?search=${encodeURIComponent(r.applicationId.slice(0, 8))}&open=${encodeURIComponent(r.applicationId)}`} className="font-medium text-ink-900 hover:underline">{r.candidateName}</Link>
                ...
                <td className="py-2 pr-3 text-right"><span className="font-black text-ink-900">{r.total}/100</span><p className="text-xs text-slate-500">{r.gradedCount}/{r.assessmentCount} graded{!r.complete ? " · provisional" : ""}</p></td>
              </tr>
            ))}
